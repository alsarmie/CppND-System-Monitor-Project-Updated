...
float LinuxParser::MemoryUtilization() {
  ...
  // Used Mem = (MemTotal-(MemFree + Buffers + Cached))/MemTotal <- not in percentage as that is handled by ncurses_display.cpp
  //Cached=Cached + SReclaimable - Shmem

  string line, key, unit;
  
  float mem_total{0.0}, mem_free{0.0},buffers{0.0},cached{0.0},srec{0.0},shmem{0.0};

  float mem_utilization{0.0};
  string keyword[6]{"MemTotal", "MemFree","Buffers","Cached","SReclaimable","Shmem"};
  ...
  if (stream.is_open()) {
    while (getline(stream, line)) {
      ...
      if (line.find("MemTotal") != std::string::npos)
        // MemTotal:
        ss >> key >> mem_total >> unit;
      if (line.find("MemFree") != std::string::npos)
        // MemFree:
        ss >> key >> mem_free >> unit;

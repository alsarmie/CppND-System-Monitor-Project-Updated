...
float LinuxParser::MemoryUtilization() {
  ...
  if (stream.is_open()) {
    ...
    while (getline(stream, line)) {
      ...
      ss >> key >> value >> unit;
      auto it = std::find(memory.keywords.begin(),memory.keywords.end(),value);
  ...
  total_cached = memory.values[3]+memory.vales[5]-memory.values[4];
  mem_utilization = (memory.values[0] - (mem_free)) / memory.values[0];

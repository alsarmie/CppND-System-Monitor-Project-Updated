...
vector<long long> LinuxParser::CpuUtilization() {
  ...
  if (stream.is_open()) {
    ...
    line = line.substr(2);
    std::istringstream ss(line);
    ss >> values[CPUFields::kUser_] >>  values[CPUFields::kUser_]>> values[CPUFields::kUser_]>> values[CPUFields::kUser_]>> values[CPUFields::kUser_]>> values[CPUFields::kUser_]>> values[CPUFields::kUser_];
  }
  return values;
}

...
float Processor::Utilization(int kCPU) {
  ...
  idle_ =
      per_core_values[kCPU][CPUFields::kIdle_] + per_core_values[kCPU][CPUFields::kIOwait_];
  ...
  non_idle_ =
      per_core_values[kCPU][CPUFields::kUser_] + per_core_values[kCPU][CPUFields::kNice_] +
      per_core_values[kCPU][CPUFields::kSystem_] + per_core_values[kCPU][CPUFields::kIRQ_] +
      per_core_values[kCPU][CPUFields::kSoftIRQ_] + per_core_values[kCPU][CPUFields::kSteal_];
  prev_total_ = prev_idle_ + prev_non_idle_;
  total_ = idle_ + non_idle_;
  delta_total_ = total_ - prev_total_;
  delta_idle_ = idle_ - prev_idle_;
  ...
  //  History
  std::copy(std::begin(agg_cpu_values), std::end(per_core_values[kCPU]),
